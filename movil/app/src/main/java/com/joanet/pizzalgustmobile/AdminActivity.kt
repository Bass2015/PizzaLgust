        ...
        if (authToken != null) {
            ...
            call.enqueue(object : Callback<DeleteUserResponse> {
                override fun onResponse(call: Call<DeleteUserResponse>, response: Response<DeleteUserResponse>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@AdminActivity, "Usuario borrado con éxito.", Toast.LENGTH_SHORT).show()
                    } else {
                        Toast.makeText(this@AdminActivity, "Error: ${response.code()}", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(call: Call<DeleteUserResponse>, t: Throwable) {
                    Toast.makeText(this@AdminActivity, "Error en la red: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
        } else {
